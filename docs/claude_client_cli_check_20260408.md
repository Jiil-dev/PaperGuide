...
`-p`는 별도 서브커맨드가 아니라 옵션 플래그. `claude --help`와 동일한 출력.

## 3. 플래그 매핑 확인

| 설계의 의도 | 설계에 쓴 플래그 | 실제 존재하는 플래그 | 일치 |
|---|---|---|---|
| 시스템 프롬프트 | `--system-prompt` | `--system-prompt <prompt>` — "System prompt to use for the session" | **일치** |
| 시스템 프롬프트 (추가) | `--append-system-prompt` | `--append-system-prompt <prompt>` — "Append a system prompt to the default system prompt" | **존재** (설계에서는 미사용) |
| JSON schema | `--json-schema` | `--json-schema <schema>` — "JSON Schema for structured output validation. Example: ..." | **일치** |
| CLAUDE.md 건너뛰기 | `--bare` | `--bare` — "Minimal mode: skip hooks, LSP, plugin sync, attribution, auto-memory, background prefetches, keychain reads, and CLAUDE.md auto-discovery." | **일치** |
| JSON 출력 | `--output-format json` | `--output-format <format>` — choices: "text", "json", "stream-json" | **일치** |
| 도구 비활성화 | (설계에 없음) | `--tools ""` — "Use \"\" to disable all tools" | **추가 검토** |

## 4. 설계 보정 필요 여부

**모든 핵심 플래그가 정확히 일치. 보정 불필요.**

추가 발견 사항:

1. **`--tools ""`로 도구 비활성화 가능**: 우리 프로젝트에서는 Claude가 파일 편집, Bash 실행 등을 하면 안 됨. `--bare`만으로 충분한지, 추가로 `--tools ""`을 넣어야 하는지는 실제 테스트 시 확인. `--bare` 설명에 "tool access" 비활성화는 명시되어 있지 않으므로, 안전하게 `--tools ""`을 추가하는 것을 권장.

2. **`--no-session-persistence`**: "sessions will not be saved to disk and cannot be resumed (only works with --print)". 우리는 일회성 호출이므로 세션 저장이 불필요. 디스크 노이즈를 줄이기 위해 이 플래그도 추가 권장.

3. **`--system-prompt` vs `--append-system-prompt`**: 설계에서 `--system-prompt`을 선택한 것이 맞음. `--system-prompt`은 기본 시스템 프롬프트를 **대체**하고, `--append-system-prompt`은 기본에 **추가**. 우리는 커스텀 시스템 프롬프트를 완전히 제어하므로 `--system-prompt`이 올바른 선택.
...
`--tools ""`와 `--no-session-persistence` 추가만이 유일한 보정 사항. 핵심 플래그는 설계 그대로.
